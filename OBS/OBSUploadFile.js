 ...
 * @LastEditors  : hxz
 * @LastEditTime : 2020-01-07 15:07:20
 */
...
function checkType(file) {
  ...
  const map = ['image/jpeg', 'image/jpeg', 'image/jpeg', 'image/png', 'image/gif']
  let attrs = null
  legalType.some((v, i) => {
    if (_.endsWith(file, v)) {
...
const OBSupload = function(dir, filePath) {
  ...
  let { access, expires_at, secret, securitytoken, bucket, endpoint } = store.state.Home.security
  ...
  const OBSPolicy = {
    ...
    conditions: [
      ...
      { success_action_redirect: http.config.baseUrl + '/uploadimage/putUploadParams' }
    ]
  }
